...
** An Amazon EC2 instance that acts as a mirror arbiter in Availability Zone 3 (not shown). The arbiter checks the health of the two mirror nodes and detects when a node is offline.
...
*The cloud development kit that deploys this Quick Start does not cover the components marked by asterisks because this requires a design decision on how to connect to the SWIFT network.
